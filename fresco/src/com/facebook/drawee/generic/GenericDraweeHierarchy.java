...
public class GenericDraweeHierarchy implements SettableDraweeHierarchy {
  ...
  GenericDraweeHierarchy(GenericDraweeHierarchyBuilder builder) {
    ...
    if (numOverlays > 0) {
      ...
      if (builder.getPressedStateOverlay() != null) {
        layers[overlaysIndex + index++] = maybeApplyRoundingPressedBitmap(mRoundingParams, mResources, builder.getPressedStateOverlay());
      }
    }
  ...
  private static Drawable maybeApplyRoundingPressedBitmap(
      @Nullable RoundingParams roundingParams,
      Resources resources,
      Drawable drawable) {
      if (roundingParams != null &&
          roundingParams.getRoundingMethod() == RoundingParams.RoundingMethod.BITMAP_ONLY) {
        Drawable roundingDrawable = applyRounding(roundingParams, resources, drawable);
        drawable = roundingDrawable;
      }
