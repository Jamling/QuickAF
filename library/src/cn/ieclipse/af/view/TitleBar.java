...
public class TitleBar extends LinearLayout {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        addView(mLeftContainer, new LinearLayout.LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT));
        addView(mMiddleContainer, new LinearLayout.LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT));
        addView(mRightContainer, new LinearLayout.LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT));
    }
    ...
    public void setGravity(int gravity) {
        if (mGravity != gravity) {
            ...
            if (mMiddleContainer != null) {
                mMiddleContainer.setGravity((gravity & Gravity.HORIZONTAL_GRAVITY_MASK) | Gravity.CENTER_VERTICAL);
            }
            requestLayout();
        }
    }
     ...
     * @since 3.0.0
     */
    public View createItem(@LayoutRes int layout) {
        View v = LayoutInflater.from(getContext()).inflate(layout, mRightContainer, false);
        return v;
    }

    private ViewGroup.LayoutParams getItemLayoutParam(View view, LinearLayout parent) {
        ViewGroup.LayoutParams p = view.getLayoutParams();
        if (p == null) {
            p = new LinearLayout.LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
        }
        if (parent.getChildCount() > 0 && p instanceof MarginLayoutParams) {
    ...
    public void addLeft(View view) {
        ViewGroup.LayoutParams p = getItemLayoutParam(view, mLeftContainer);
        mLeftContainer.addView(view, p);
    }

    public void addLeft(View view, ViewGroup.LayoutParams params) {
        ViewGroup.LayoutParams p = params == null ? getItemLayoutParam(view, mLeftContainer) : params;
        mLeftContainer.addView(view, p);
    }
    ...
    public void addMiddle(View view) {
        ViewGroup.LayoutParams p = getItemLayoutParam(view, mMiddleContainer);
    ...
    public void addRight(View view) {
        ViewGroup.LayoutParams p = getItemLayoutParam(view, mRightContainer);
        mRightContainer.addView(view, p);
    }

    public void addRight(View view, ViewGroup.LayoutParams params) {
        ViewGroup.LayoutParams p = params == null ? getItemLayoutParam(view, mRightContainer) : params;
        mRightContainer.addView(view, p);
    }

    public View addRight(@LayoutRes int layout) {
        View v = createItem(layout);
        addRight(v);
        return v;
    }
...
//                / config.getSumWeight());
        int maxWidth = (width >> 1) - config.padding;

        mLeftContainer.measure(MeasureSpec.makeMeasureSpec(maxWidth, MeasureSpec.AT_MOST),
...
//                / config.getSumWeight());
        int maxWidth = (width >> 1) - config.padding;
        mRightContainer.measure(MeasureSpec.makeMeasureSpec(maxWidth, MeasureSpec.AT_MOST),
            MeasureSpec.makeMeasureSpec(0, MeasureSpec.UNSPECIFIED));
        rightHeight = Math.max(rightHeight, mRightContainer.getMeasuredHeight());
    ...
    private void measureMiddle(int width, int height) {
        middleWidth = 0;
        middleHeight = 0;
        leftWidth = mLeftContainer.getMeasuredWidth();
        rightWidth = mRightContainer.getMeasuredWidth();
        int maxWidth = 0;// (int) (width * config.middleWeight / config.getSumWeight());
        ...
        mMiddleContainer.measure(MeasureSpec.makeMeasureSpec(maxWidth, MeasureSpec.EXACTLY),
            MeasureSpec.makeMeasureSpec(0, MeasureSpec.UNSPECIFIED));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (bottomDrawable != null) {
            bottomDrawable.setBounds(0, getMeasuredHeight() - bottomDrawableHeight, getMeasuredWidth(),
                getMeasuredHeight());
